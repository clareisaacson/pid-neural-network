...
class NeuralNet:
  def __init__(self, num_hidden1, num_hidden2, learning_rate):
    ...
    Initialize weights to random values in range [-0.5, 0.5]
    ...
    self.hidden1 = []
    self.hidden2 = []
    self.weights1 = ((np.random.rand(self.num_in, self.num_hid1)) * 2) - 1
    self.weights2 = ((np.random.rand(self.num_hid1, self.num_hid2))  * 2) - 1
    self.weights3 = ((np.random.rand(self.num_hid2, self.num_out)) * 2) - 1
  ...
  def back_propagate(self, target):
    ...
    for k in range(self.num_hid1):
        ...
        second_step_errors.append(unit_error)

    # Change weights between hidden layer 1 and input layer
  ...
  def train(self, X, Y, iterations=30):
    ...
    :return: None
    """
    print 'Training PID NN for %d iterations...' %iterations
    print 'On %d observations.' %len(X)
